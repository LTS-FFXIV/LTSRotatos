...
namespace KirboRotations.Utility.Reflection;
...
public static class DalamudReflector
{
    ...
    public static bool TryGetLocalPlugin(out object localPlugin, out Type type)
    {
        try
        {
            if (ECommons.ECommonsMain.Instance == null)
            {
            ...
            foreach (var t in installedPlugins)
            {
                if (t != null)
                {
                    ...
                    if (object.ReferenceEquals(type.GetField("instance", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(t), ECommons.ECommonsMain.Instance))
                    {
                        localPlugin = t;
                        return true;
                    }
