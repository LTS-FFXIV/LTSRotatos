...
public class BRD_LTS : BRD_Base
{
    ...
    public override string RotationName => "LTS BRD";

    public override string Description => Helper.GetSourceCode + "Please make sure that the three song times add up to 120 seconds!";
    ...
    protected override bool GeneralGCD(out IAction act)
    {
        ...
        if (PvP_ApexArrow.CanUse(out act, CanUseOption.MustUse)) return true;

        if (PvP_PitchPerfect.CanUse(out act)) return true;
    ...
    protected override bool AttackAbility(out IAction act)
    {
        ...
        if (PvP_SilentNocturne.CanUse(out act)) return true;
        if (PvP_TheWardensPaean.CanUse(out act)) return true;

        if (PvP_EmpyrealArrow.CanUse(out act, CanUseOption.EmptyOrSkipCombo)) return true; 
        
        if (PvP_RepellingShot.CanUse(out act)) return true;
        #endregion
