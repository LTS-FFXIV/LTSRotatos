...
public sealed class WAR_Supporters : WAR_Base
{
    ...
    public override string RotationName => "LTS Warrior";

    public override string Description => Helper.GetSourceCode;
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        if (remainTime <= CountDownAhead)
        {
            ...
            {
                if (Onslaught.CanUse(out var act1, CanUseOption.IgnoreClippingCheck)) return act1;
            }
        }
    ...
    {
        byte aoeCount = IsInHighEndDuty ? (byte)3 : (byte)2;
        ...
        if (!Player.WillStatusEndGCD(3, 0, true, StatusID.SurgingTempest))
        {
            ...
            {
                if (SteelCyclone.CanUse(out act, aoeCount: aoeCount)) return true;
                if (InnerBeast.CanUse(out act)) return true;
            }
        }

        if (MythrilTempest.CanUse(out act, aoeCount: aoeCount)) return true;
        if (Overpower.CanUse(out act, aoeCount: aoeCount)) return true;
    ...
    protected override bool DefenseSingleAbility(out IAction act)
    {
        ...
        {
            //10
            if (RawIntuition.CanUse(out act, CanUseOption.OnLastAbility) && NumberOfHostilesInRange >= 3) return true;
            if (RawIntuition.CanUse(out act)) return false;
    ...
    private static bool HighDefense(out IAction act)
    {
        //30
