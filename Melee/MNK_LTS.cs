...
public class MNK_Supporters : MNK_Base
{
    ...
    public override string RotationName => "LTS Lunar Solar S";
    ...
    && (!RiddleOfFire.EnoughLevel ||
    Player.HasStatus(true, StatusID.RiddleOfFire) && !Player.WillStatusEndGCD(2, 0, true, StatusID.RiddleOfFire)
    || RiddleOfFire.WillHaveOneChargeGCD(1) && RiddleOfFire.ElapsedAfter(60)))
        {
            if (PerfectBalance.CanUse(out act, CanUseOption.EmptyOrSkipCombo)) return true;
        }

        if (Player.HasStatus(true, StatusID.RiddleOfFire)
            && !Player.WillStatusEnd(0, true, StatusID.RiddleOfFire)
            && Brotherhood.CanUse(out act)) return true;
